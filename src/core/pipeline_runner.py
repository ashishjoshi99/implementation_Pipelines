...
def read_source(spark: SparkSession, source_cfg: dict) -> DataFrame:
    """Reads data strictly from the Unity Catalog table."""
...
def apply_transformations(df: DataFrame, transformations: list) -> DataFrame:
    """Dynamically applies transformations defined in the config."""
    for t_config in transformations:
        ...
        # Get the function and invoke it with DataFrame and any additional kwargs from config
...
def write_target(spark: SparkSession, df: DataFrame, config: dict):
    """Writes data to the target Unity Catalog table using the specified mode."""
    ...
    # Some writes (SCD) need the spark session and join keys
...
def run_pipelines(spark: SparkSession, config_path: str):
    """Executes all Medallion pipelines defined in the YAML file."""
    with open(config_path, 'r') as file:
        config_data = yaml.safe_load(file)
        
    # Support multiple pipelines or fallback to a single 'pipeline'
